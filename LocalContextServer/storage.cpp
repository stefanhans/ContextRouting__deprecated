...
OfferAdress::OfferAdress(unsigned char* addressId, struct sockaddr_in offerAddress) {
	printf("OfferAdress\n");
...
int ContextStorage::storeContextStruct(ContextStruct *contextStruct) {
	...
	if(contextStruct->contextData->command[0] == CMD_OFFER) {
		...
		printContextStruct(contextStruct);
	    //		printContextStorage(this);
...
int ContextStorage::deserializeStruct(char *buffer, int bytes, ContextStruct *contextStruct) {
	...
	size_t s;
	for (s = 0; s < sizeof(contextStruct->sourceUuid) - 1; s++) {
		printf("%02x", contextStruct->sourceUuid[s]);
	}
	putchar('\n');

	contextStruct->channel = buffer[16];
	printf("context->channel\t: %u\n", contextStruct->channel);
...
void printOfferAdresses(ContextStorage *storage) {
	printf("printOfferAdresses\n");

	int i = 0;
	for (std::vector<OfferAdress*>::iterator iter = storage->offerAddresses.begin();
			iter != storage->offerAddresses.end(); ++iter) {

		printf("offerAddresses[%u]->addressId\t: ", i);
		size_t j;
		for (j = 0; j < sizeof((*iter)->addressId) - 1; j++) {
			printf("%02x", (*iter)->addressId[j]);
		}
		putchar('\n');

		printf("offerAddresses[%u]->addressId.sin_addr\t: %s\n", i, inet_ntoa((*iter)->offerAddress.sin_addr));
		printf("offerAddresses[%u]->addressId.sin_port\t: %u\n", i, ntohs((*iter)->offerAddress.sin_port));
