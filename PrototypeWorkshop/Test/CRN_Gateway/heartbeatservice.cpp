...
int HeartbeatService::processUDP(void* receivedPacket, int socket, void *buffer, size_t size, struct sockaddr *addr) {
	...
	std::cout << getUuidString(*((ContextPacket*) receivedPacket)->getUuid()) << " : " << "Service Heartbeat provided: HEARTBEAT CIP with client's ip:port and current timestamp." << std::endl;
