...
int ContextStorage::deserializeStruct(char *buffer, int bytes, ContextStruct *contextStruct) {
	...
	contextStruct->contextData->command[0] = buffer[16];
	contextStruct->contextData->command[1] = buffer[17];

	contextStruct->contextData->datatype = buffer[18];

	int i, j;
	for (i=0, j=19; j<159; i++, j++) {
		contextStruct->contextData->data[i] = buffer[j];
	}

	contextStruct->contextData->contextMetatId->context = buffer[159];
	contextStruct->contextData->contextMetatId->mask    = buffer[160];

	contextStruct->contextArray->contextId->context = buffer[161];
	contextStruct->contextArray->contextId->mask    = buffer[162];

	int k;
	for (k=163;k<bytes; k+=2) {
